...
BEGIN
    ...
    SELECT v_admin_role_id, p.id, 'administrador'
    FROM seguridad.permisos p
    ON CONFLICT (id_rol, id_permiso) DO NOTHING;
    
    -- Cliente role - limited permissions
    INSERT INTO seguridad.rol_permisos (id_rol, id_permiso, nivel)
    SELECT v_cliente_role_id, p.id, 'lectura'
    FROM seguridad.permisos p
    ...
    SELECT v_inventario_role_id, p.id, 
        CASE 
            WHEN p.nombre LIKE 'inventario%' THEN 'escritura'
            ELSE 'lectura'
        END
    FROM seguridad.permisos p
    ...
    SELECT v_finanzas_role_id, p.id, 
        CASE 
            WHEN p.nombre LIKE 'finanzas%' THEN 'escritura'
            ELSE 'lectura'
        END
    FROM seguridad.permisos p
