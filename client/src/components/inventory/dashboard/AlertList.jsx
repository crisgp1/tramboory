      ...
      <div className="bg-white rounded-xl shadow-sm p-5 h-full border border-gray-100">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            ...
            <h3 className="text-base font-semibold ml-2">{title}</h3>
          </div>
          
          {items.length > 0 && viewAllLink && (
            <Link 
              to={viewAllLink} 
              className="text-indigo-600 hover:text-indigo-800 text-xs font-medium flex items-center px-2 py-1 rounded-full hover:bg-indigo-50 transition-colors"
            >
              Ver todos
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-3">
            {items.slice(0, 4).map((item, index) => (
              <motion.div 
                key={item.id || index} 
                className={`border border-gray-100 rounded-lg p-3 hover:bg-gray-50 transition-colors ${onItemClick ? 'cursor-pointer' : ''}`}
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start">
                    ...
                    <div>
                      <h4 className="font-medium text-sm">{item.nombre || item.mensaje}</h4>
                      {item.materiaPrima && (
                        <p className="text-xs text-gray-500 mt-1">{item.materiaPrima.nombre}</p>
                      )}
                      {item.descripcion && (
                  ...
                  <div>
                    ...
                    {item.nivel && (
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                        item.nivel === 'crítico' ? 'bg-rose-50 text-rose-700 border border-rose-100' : 
                        item.nivel === 'advertencia' ? 'bg-amber-50 text-amber-700 border border-amber-100' :
